...
public class MarketDataRepoDataServiceImpl implements IMarketDataRepository {
    ...
    private GatewayDsmProvider gdp;

    public MarketDataRepoDataServiceImpl(GatewayDsmProvider gdp) {
        this.gdp = gdp;
    }
    ...
    public List<BarField> loadBars(ChannelType channelType, String unifiedSymbol, LocalDate startDate, LocalDate endDate) {
        ...
        try {
            return gdp.getDsmFactory(channelType).getMinutelyData(unifiedSymbol, startDate, endDate);
        } catch (Exception e) {
    ...
    public List<BarField> loadDailyBars(String gatewayId, String unifiedSymbol, LocalDate startDate, LocalDate endDate) {
        ...
        try {
            return gdp.getDsmFactory(ChannelType.valueOf(gatewayId)).getDailyData(unifiedSymbol, startDate, endDate);
        } catch (Exception e) {
            log.warn("第三方数据服务暂时不可用：{}", e.getMessage(), e);
        }
        return Collections.emptyList();
    ...
    public List<LocalDate> findHodidayInLaw(String gatewayType, int year) {
        ...
        try {
            resultList = gdp.getDsmFactory(ChannelType.valueOf(gatewayType)).getHolidays(ExchangeEnum.SHFE, LocalDate.of(year, 1, 1), LocalDate.of(year, 12, 31));
        } catch (Exception e) {
            log.warn("第三方数据服务暂时不可用：{}", e.getMessage(), e);
            return Collections.emptyList();
        }
