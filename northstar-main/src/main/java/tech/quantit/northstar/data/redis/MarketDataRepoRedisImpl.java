...
public class MarketDataRepoRedisImpl extends MarketDataRepoDataServiceImpl {
    ...
    public MarketDataRepoRedisImpl(RedisTemplate<String, byte[]> redisTemplate, GatewayDsmProvider gdp) {
        super(gdp);
    ...
    public List<BarField> loadBars(ChannelType channelType, String unifiedSymbol, LocalDate startDate, LocalDate endDate0) {
        ...
        if (endDate.isAfter(startDate)) {
            List<BarField> list = super.loadBars(channelType, unifiedSymbol, startDate, endDate).stream().sorted((a, b) -> a.getActionTimestamp() < b.getActionTimestamp() ? -1 : 1).toList();
        ...
        if (resultList.isEmpty()) {
            ...
            if (resultList.peekLast().getTradingDay().equals(today.format(DateTimeConstant.D_FORMAT_INT_FORMATTER)) || today.getDayOfWeek().getValue() > 5) {
                do {
                    localQueryDate = localQueryDate.plusDays(1);
    ...
    private List<BarField> findBarData(LocalDate date, ChannelType channelType, String unifiedSymbol) {
        ...
        return Optional.ofNullable(list.range(0, list.size())).orElse(Collections.emptyList()).stream().map(this::convert).filter(Objects::nonNull).toList();
    }
