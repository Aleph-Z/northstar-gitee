...
public class TradeIntent implements TransactionAware, TickDataAware {
	...
	public synchronized void onTick(TickField tick) {
		...
		} else if (orderIdRef.isPresent() && context.getState().isOrdering() && context.isOrderWaitTimeout(orderIdRef.get(), timeout) && tick.getActionTimestamp() - lastCancelReqTime > 3000) {
